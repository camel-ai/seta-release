"""
seta_env miles training: reward, metrics, and rollout class.

The generate function is provided by:
    miles.rollout.generate_hub.agentic_tool_call.generate
with --custom-agent-function-path pointing to seta_agent_function.run

Task-type agnostic — reward is pre-computed by env_service during
TerminalEnvironment.step() and stored in sample.metadata["reward"].
