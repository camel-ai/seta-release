"""seta_env miles training launcher (Qwen3-8B).

Pure training-side configuration. Environment config (agent, TITO, Docker,
dataset) lives in env_service started separately.

Usage:
    python scripts/miles/run.py
    python scripts/miles/run.py --mode debug_rollout_only
    python scripts/miles/run.py --prompt-data /path/to/seta_train.jsonl
"""
...
class ScriptArgs(U.ExecuteTrainConfig):
    ...
    save_dir: str = "/root/Qwen3-8B_seta_agent/"
    max_seq_len: int = 32768
    prompt_data: str = str(SCRIPT_DIR / "seta_train.jsonl")

    # W&B settings
    wandb_key: str = os.environ.get("WANDB_KEY", os.environ.get("WANDB_API_KEY", ""))
    wandb_project: str = os.environ.get("WANDB_PROJECT", "seta-agent-training")
    wandb_team: str = os.environ.get("WANDB_TEAM", "")
    wandb_run_name: str = "qwen3-8b-seta-tito"
...
def execute(args: ScriptArgs):
    ...
    agent_args = (
        ...
        "--custom-agent-function-path seta_agent_function.run "
    ...
    # env_service is started separately — only set AGENT_SERVER_URL
    # so that seta_agent_function.run() knows where to POST
    extra_env_vars = {
        "PYTHONPATH": f"{args.megatron_path}:{SCRIPT_DIR}:{miles_root}",
        "MILES_EXPERIMENTAL_ROLLOUT_REFACTOR": "1",
        "AGENT_SERVER_URL": os.environ.get("AGENT_SERVER_URL", "http://localhost:8002"),
        "MILES_HOST_IP": os.environ.get("MILES_HOST_IP", socket.gethostname()),
    }
